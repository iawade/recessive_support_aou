...
def boxplot(
    ...
    p = (
        ...
        + geom_boxplot()
        + labs(title=f"Comparing distributions of EHR {c_name} measures", x="EHR site")
...
def pid_level_summary(m_cids, clean_lab_df, base_cols = ['person_id', 'value_as_number'], save_file = True
    ...
    if save_file == True:
        ...
        df_final.to_csv(f'{BUCKET}/notebooks/phenotype_data/{filename}', index=False)  
        print(f"\n'{filename}' saved to the bucket under 'notebooks/phenotype_data/'.") 
